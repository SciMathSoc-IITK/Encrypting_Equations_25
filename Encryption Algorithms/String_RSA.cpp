...
string p = "55094911065644703956031264108083761730534395020799";
string q = "11242835594999365134474458320592006554078556102111659";
string e = "99999999999999999999999999999999999999999999999989"; // Standard RSA public exponent
...
int main() {
    //Intializing public key
    string N = String_Multiplication(p, q);
    cout << "N = " << N << endl;
    string Phi_N = String_Multiplication(String_Subtraction(p,"1"), String_Subtraction(q,"1"));
    cout << "Phi(N) = " << Phi_N << endl;

    // Check if e and Phi(N) are coprime
    string gcd_check = String_GCD(e, Phi_N);
    cout << "GCD(e, Phi(N)) = " << gcd_check << endl;

    //Checking if choise of e is valid.
    if (gcd_check != "1") {
        cout << "Error: e and Phi(N) are not coprime!" << endl;
        return 1;
    }

    //Checking if the value of d is calculated correctly.
    string d = String_Modulo_Inverse(e, Phi_N);
    cout << "d = " << d << endl;

    // Verify that e*d ≡ 1 (mod Phi(N))
    string verification = String_Modulo(String_Multiplication(e, d), Phi_N);
    cout << "e*d mod Phi(N) = " << verification << endl;

    if (verification == "1") {
        cout << "SUCCESS: RSA key pair is valid!" << endl;
    } else {
        cout << "ERROR: RSA key pair is invalid!" << endl;
    }

    // Test encryption/decryption with a small message
    string message;
    cin >> message;
    string encrypted = String_Modulo_Exponentiation(message, e, N);
    string decrypted = String_Modulo_Exponentiation(encrypted, d, N);
